...
//				lstTypes = materialTypeRepository.findByNmaterialtypecodeNotAndNstatusOrderByNmaterialtypecode(-1, 1);
					...
					if (!lstCategories.isEmpty()) {
						...
						if (!lstMaterials.isEmpty()) {
							lstMaterialInventories = elnmaterialInventoryRepository
									.findByMaterialAndNtransactionstatusAndIsresuableOrderByNmaterialinventorycodeDesc(
											lstMaterials.get(0), 28,false);
							lstMaterialInventories.addAll(elnmaterialInventoryRepository
									.findByMaterialAndNtransactionstatusAndIsresuableAndIsresuablereachedOrderByNmaterialinventorycodeDesc(
											lstMaterials.get(0), 28,true,false));
						}
				...
				lstMaterialInventories = elnmaterialInventoryRepository
						.findByMaterialAndNtransactionstatusAndIsresuableOrderByNmaterialinventorycodeDesc(objMaterial, 28,false);
				lstMaterialInventories.addAll(elnmaterialInventoryRepository
						.findByMaterialAndNtransactionstatusAndIsresuableAndIsresuablereachedOrderByNmaterialinventorycodeDesc(objMaterial, 28,true,false));
				...
				lstMaterialInventories.add(elnmaterialInventoryRepository
						.findByNmaterialinventorycodeAndNtransactionstatusAndIsresuable(nmaterialinventorycode,28,false));
				lstMaterialInventories.add(elnmaterialInventoryRepository
						.findByNmaterialinventorycodeAndNtransactionstatusAndIsresuableAndIsresuablereached(nmaterialinventorycode,28,true,false));
	...
	public ResponseEntity<Object> getResultInventoryTransaction(Map<String, Object> inputMap)
		...
		if(objInventory.getMaterial().getReusable()) {
			
			mapJsonData.put("reusable", true);
			...
			mapJsonData.put("Total Quantity", totalQuantity);
			mapJsonData.put("NotificationQty", objInventory.getNqtynotification());
			mapJsonData.put("reusable", false);
...
//				objTransaction.setCreateddate(objCreatedDate);
	...
	public ResponseEntity<Object> createMaterialResultUsed(Map<String, Object> inputMap) throws JsonParseException, JsonMappingException, IOException {
		...
		if(objInventory.getMaterial().getReusable()) {
			...
			Double getIssuedQty = new Double(0);
			Double getUsedQty =  new Double(0);
			Double getQtyLeft =  new Double(0);
...
//			resultUsedMaterial.setCreateddate(cft.getTransactiondate());
			...
			if(objInventory.getIsresuable()) {
				objInventory.setReusableqty(objInventory.getReusableqty()+1);
				objInventory.setIsresuablereached(objInventory.getReusablecount().equals(objInventory.getReusableqty()));
			}
