...
//		}
	...
	public ResponseEntity<Object> createElnMaterialInventory(Map<String, Object> inputMap) throws Exception {
		...
		/**
		 * Old LIMS inventory resuable code delete dont delete
		 * new resusable changes as per requirement
		 * 
		 * Done by Sathishkumar chandrasekar 15-11-2024 
		 */
//		if (isReusable) {
...
//		else {
			...
			if(isReusable) {
				objInventory.setIsresuable(true);
				objInventory.setReusablecount(objInventory.getMaterial().getReusableqty());
				objInventory.setReusableqty(0);
				objInventory.setIsresuablereached(false);
			}else {
				objInventory.setIsresuable(false);
			}
...
//		List<ElnmaterialInventory> inventoryItems = elnmaterialInventoryReppository.findByMaterialIn(lstMaterial);
	...
	public ResponseEntity<Object> getElnMaterialInventoryByMaterial(List<Integer> lstMaterial) {
		...
		List<ElnmaterialInventory> inventoryItems = new ArrayList<ElnmaterialInventory>();
				
		inventoryItems.addAll(elnmaterialInventoryReppository.findByMaterialInAndNtransactionstatusAndNstatusAndIsresuableOrderByNmaterialinventorycodeDesc(material,28,1,false));
		inventoryItems.addAll(elnmaterialInventoryReppository.findByMaterialInAndNtransactionstatusAndNstatusAndIsresuableAndIsresuablereachedOrderByNmaterialinventorycodeDesc(material,28,1,true,false));
